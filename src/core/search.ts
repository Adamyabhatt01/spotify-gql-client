...
class SpotifySearchEndpoint {
  ...
  async convertTracks(tracks: Record<string, any>[]): Promise<Track[]> {
    const ids = tracks
      .filter(
        (item) =>
  ...
  async all(
    ...
    const tracks = await this.convertTracks(searchData.tracksV2.items);
  ...
  async tracks(
    ...
    const pagingInfo = searchData.pagingInfo;
    const items = await this.convertTracks(searchData.items);
